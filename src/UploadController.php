...
class UploadController extends Controller
{
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $path       = $file->storeAs($savePath, $uniqueName, config('ibrand.uploader.default.disk'));
            if (!Storage::disk(config('ibrand.uploader.default.disk'))->has($path)) {
            ...
            UploadFiles::create([
                'client_id' => $client_id,
                'path'      => $path,
                'url'       => Storage::disk(config('ibrand.uploader.default.disk'))->url($path),
            ]);

            return response()->json(['status' => true, 'message' => '上传成功', 'data' => ['path' => $path, 'url' => Storage::disk(config('ibrand.uploader.default.disk'))->url($path)]]);
